            ...
            foreach (XElement itemsSection in itemsSections)
            {
                foreach (XElement xElement in itemsSection.Elements())
                {
                    ...
                    if (item.Include == null)
                    {
                        ...
                        //todo - figure out what 'Update' is
                    }

                    //hack this in for now
                    if (xElement.Name.LocalName == "PackageReference")
                    {
                        item.Version = xElement.Attributes().FirstOrDefault(x => x.Name.LocalName == XmlNames.Version)?.Value;
                    }
